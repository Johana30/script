...
update_eus_db() {
  if [[ -n "$(command -v jq)" ]]; then
    ...
    eus_database_move
    script_total_runs="$(jq -r '.scripts."'"${script_name}"'".total_runs' "${eus_dir}/db/db.json")"
...
support_file() {
  ...
  if dpkg -l lsb-release 2> /dev/null | grep -iq "^ii\\|^hi"; then lsb_release -a &> "/tmp/EUS/support/lsb-release"; else cat /etc/os-release &> "/tmp/EUS/support/os-release"; fi
  if [[ -n "$(command -v jq)" ]]; then
  ...
  dpkg -l | grep "mongo\\|oracle\\|openjdk\\|unifi\\|temurin" &> "/tmp/EUS/support/unifi-packages-list"
  dpkg -l &> "/tmp/EUS/support/dpkg-packages-list"
  journalctl --since yesterday &> "/tmp/EUS/support/journal.log"
  #
  lsblk -iJ -fs &> "/tmp/EUS/support/disk-layout.json"
  if [[ -n "$(command -v sar)" && -n "$(command -v sadf)" && -n "$(command -v jq)" ]]; then
    systat_temp="$(mktemp --tmpdir=/tmp/EUS sysstat_XXXXX)"
    sar -r 1 1 -o "${systat_temp}" &> /dev/null
    sadf "${systat_temp}" -j -- -u -r -n DEV | jq &> "/tmp/EUS/support/sysstat.json"
    rm --force "${systat_temp}" &> /dev/null
        ...
        jq -n \
          --argjson "system" "$( 
            jq -n \
              ...
              "{ \"hostname\" : \"$system_hostname\", \"kernel-name\" : \"$system_kernel_name\", \"kernel-release\" : \"$system_kernel_release\", \"kernel-version\" : \"$system_kernel_version\", \"machine\" : \"$system_machine\", \"hardware\" : \"$system_hardware\", \"operating-system\" : \"$system_os\" }" \
              '$ARGS.named'
            )" \
          '$ARGS.named' \
        jq -n \
  ...
  if dpkg -l tar 2> /dev/null | grep -iq "^ii\\|^hi"; then
    ...
    if [[ -n "$(command -v jq)" && -f "${eus_dir}/db/db.json" ]]; then
      jq '.scripts."'"${script_name}"'" |= . + {"support": {("'"${support_file_name}"'"): {"abort-reason": "'"${abort_reason}"'","upload-results": ""}}}' "${eus_dir}/db/db.json" > "${eus_dir}/db/db.json.tmp" 2>> "${eus_dir}/logs/eus-database-management.log"
      eus_database_move
    fi
    tar czvfh "/tmp/eus_support_${support_file_time}.tar.gz" --exclude="${eus_dir}/unifi_db" --exclude="/tmp/EUS/downloads" --exclude="/usr/lib/unifi/logs/remote" "/tmp/EUS" "${eus_dir}" "/usr/lib/unifi/logs" "/etc/apt/sources.list" "/etc/apt/sources.list.d/" &> /dev/null
    ...
    zip -r "/tmp/eus_support_${support_file_time}.zip" "/tmp/EUS/" "${eus_dir}/" "/usr/lib/unifi/logs/" "/etc/apt/sources.list" "/etc/apt/sources.list.d/" -x "${eus_dir}/unifi_db/*" -x "/tmp/EUS/downloads" -x "/usr/lib/unifi/logs/remote" &> /dev/null
  fi
  if [[ -n "${support_file}" ]]; then
    ...
    if [[ "$(jq -r '.database."support-file-upload"' "${eus_dir}/db/db.json")" == 'true' ]]; then
      upload_result="$(curl --silent -X POST -F "file=@${support_file}" "https://up.glennr.nl/upload" | jq -r '.[]')"
...
eus_create_directories() {
  for dir_name in "$@"; do
    if ! [[ -d "${eus_directory_location}/${dir_name}" ]]; then if ! mkdir -p "${eus_directory_location}/${dir_name}"; then abort_reason="Failed to create directory ${eus_directory_location}/${dir_name}."; header_red; abort; fi; fi
  done
...
eus_directories() {
  ...
  eus_directory_location="/tmp/EUS"
  eus_create_directories "upgrade" "dpkg"
...
script_option_list=(-skip --skip --skip-swap --add-repository --local --local-controller --local-install --custom-url --help --v6 --ipv6 --email --mail --fqdn --domain-name --server-ip --server-address --retry --external-dns --force-renew --renew --dns --dns-challenge --dns-provider --dns-provider-credentials --support-file)
...
add_glennr_mongod_repo() {
  ...
  mongod_armv8_v="$(dpkg -l | grep "mongod-armv8" | grep -i "^ii\\|^hi" | awk '{print $3}' | sed 's/\.//g' | sed 's/.*://' | sed 's/-.*//g' | sed 's/+.*//g' | sort -V | tail -n 1)"
  ...
  if [[ -n "${mongod_version_major_minor}" ]]; then
    if ! [[ -s "/etc/apt/keyrings/apt-glennr.gpg" ]]; then
      ...
      if [[ "${apt_key_deprecated}" == 'true' ]]; then
        if curl -fsSL "${repo_http_https}://get.glennr.nl/apt/keys/apt-glennr.asc" | tee -a "${eus_dir}/logs/repository-keys.log" | gpg -o "/etc/apt/keyrings/apt-glennr.gpg" --dearmor &> /dev/null && [[ -s "/etc/apt/keyrings/apt-glennr.gpg" ]]; then
        ...
        if wget -qO - "${repo_http_https}://get.glennr.nl/apt/keys/apt-glennr.asc" | tee -a "${eus_dir}/logs/repository-keys.log" | apt-key add - &> /dev/null; then
          echo -e "${GREEN}#${RESET} Successfully added the key for the Glenn R. APT Repository! \\n"
        else
          abort_reason="Failed to add the key for the Glenn R. APT Repository."
          abort
...
add_mongodb_repo() {
  ...
  if [[ -n "${mongodb_version_major_minor}" ]]; then
    if ! [[ -s "/etc/apt/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" ]] || [[ "${mongodb_key_update}" == 'true' ]]; then
      ...
      aptkey_depreciated
      if [[ "${apt_key_deprecated}" == 'true' ]]; then
        if wget -qO - "${repo_http_https}://pgp.mongodb.com/server-${mongodb_version_major_minor}.asc" | tee -a "${eus_dir}/logs/repository-keys.log" | gpg --dearmor | tee -a "/etc/apt/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" &> /dev/null && [[ -s "/etc/apt/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" ]]; then
          ...
          if ! wget -qO - "${repo_http_https}://www.mongodb.org/static/pgp/server-${mongodb_version_major_minor}.asc" | tee -a "${eus_dir}/logs/repository-keys.log" | gpg --dearmor | tee -a "/etc/apt/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" &> /dev/null && [[ -s "/etc/apt/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" ]]; then
            abort_reason="Failed to add the key for MongoDB ${mongodb_version_major_minor}."
            abort
          fi
        fi
      else
        if wget -qO - "${repo_http_https}://pgp.mongodb.com/server-${mongodb_version_major_minor}.asc" | tee -a "${eus_dir}/logs/repository-keys.log" | apt-key add - &> /dev/null; then
          echo -e "${GREEN}#${RESET} Successfully added the key for MongoDB ${mongodb_version_major_minor}! \\n"
        else
          if ! wget -qO - "${repo_http_https}://www.mongodb.org/static/pgp/server-${mongodb_version_major_minor}.asc" | tee -a "${eus_dir}/logs/repository-keys.log" | apt-key add - &> /dev/null; then
            abort_reason="Failed to add the key for MongoDB ${mongodb_version_major_minor}."
            abort
          fi
        fi
...
if [[ -n "${unifi_package}" ]]; then
  ...
  jq '.scripts."'"$script_name"'" += {"recovery": {"broken-install": {"script-version": "'"$(grep -i "# Version" "${script_location}" | head -n 1 | cut -d'|' -f2 | sed 's/ //g' | cut -d'-' -f1)"'", "status": "'"$(dpkg -l | grep "unifi " | awk '{print $1}')"'", "unifi-version": "'"${broken_unifi_install_version}"'", "detected-date": "'"$(date +%s)"'"}}}' "${eus_dir}/db/db.json" > "${eus_dir}/db/db.json.tmp" 2>> "${eus_dir}/logs/eus-database-management.log"
  eus_database_move
fi
...
if ! dpkg -l procps 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  if [[ "${installing_required_package}" != 'yes' ]]; then
    install_required_packages
  fi
...
libssl_installation() {
  ...
  if [[ "${libssl_package_empty}" != 'false' ]]; then
    curl -s "${libssl_repo_url}/pool/main/o/${libssl_url_arg}/?C=M;O=D" &> /tmp/EUS/libssl.html
  ...
  if [[ "${libssl_install_success}" != 'true' ]]; then abort_reason="Failed to install libssl."; abort; fi
...
libssl_installation_check() {
  ...
  if [[ "${required_libssl_version}" == 'libssl3' ]]; then
    ...
    if ! dpkg -l libssl3 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
      libssl_install_required="true"
    elif [[ "$(dpkg-query --showformat='${Version}' --show libssl3 | sed -e 's/.*://' -e 's/-.*//g' -e 's/[^0-9.]//g' -e 's/\.//g')" -lt "${libssl_version//./}" ]]; then
      libssl_install_required="true"
    fi
    if [[ "${os_codename}" =~ (jessie|stretch|buster|bullseye|bookworm|trixie|forky) ]]; then
    ...
    if ! dpkg -l libssl1.1 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
      libssl_install_required="true"
    ...
    if ! dpkg -l libssl1.0.0 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
      libssl_install_required="true"
...
if netstat -tulpn | grep -q ":8080\\b"; then
  ...
  if [[ "$(ls -l "/proc/${port_8080_pid}/exe" 2> /dev/null | awk '{print $3}')" != "unifi" ]]; then
    ...
    eus_directory_location="/tmp/EUS"
    eus_create_directories "services"
...
change_default_ports() {
  ...
  if [[ -f /tmp/EUS/services/success_port_change && -s /tmp/EUS/services/success_port_change ]]; then
    ...
    if systemctl status unifi | grep -iq "Active: active (running)"; then
      ...
      abort_reason="Failed to start the UniFi Network Application." && abort
    fi
    sleep 3
  fi
...
script_removal() {
  header
  # Always keep the script without asking
  yes_no="Y"
}
...
while read -r mongodb_repo_version; do
  if [[ "$(curl -s https://get.glennr.nl/unifi/releases/mongodb-versions.json | jq -r '.versions."'"${mongodb_repo_version}"'".updated')" -ge "$(jq -r '.database."mongodb-key-last-check"' "${eus_dir}/db/db.json")" ]]; then
...
# Get the list of upgradeable packages and display them
...
if [[ -f /tmp/EUS/upgrade/upgrade_list ]]; then
    number_of_updates=$(wc -l < /tmp/EUS/upgrade/upgrade_list)
else
    number_of_updates='0'
fi

# Inform if no updates are available
if [[ "${number_of_updates}" == '0' ]]; then
    echo -e "${WHITE_R}#${RESET} There are no packages that need an upgrade..."
fi

echo -e "\\n${WHITE_R}----${RESET}\\n"

# Always proceed with system update without asking for user confirmation
echo -e "${WHITE_R}#${RESET} Performing the updates!"
echo -e "\\n${WHITE_R}----${RESET}\\n"
system_upgrade

# Set MongoDB packages to be installed
...
mongodb_installation_armhf() {
  aptkey_depreciated
  if [[ "${apt_key_deprecated}" == 'true' ]]; then
    if wget -qO - "${raspbian_repo_url}.public.key" | tee -a "${eus_dir}/logs/repository-keys.log" | gpg --dearmor | tee -a "/etc/apt/keyrings/raspbian.gpg" &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added key for the raspbian repository! \\n"; signed_by_value_raspbian="[ signed-by=/etc/apt/keyrings/raspbian.gpg ] "; else abort_reason="Failed to add the key for the raspbian repository."; abort; fi
  else
    if wget -qO - "${raspbian_repo_url}.public.key" | tee -a "${eus_dir}/logs/repository-keys.log" | apt-key add - &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added key for the raspbian repository! \\n"; else abort_reason="Failed to add the key for the raspbian repository."; abort; fi
  fi
    ...
    if [[ "${mongodb_org_version_no_dots::2}" == '44' && "$(echo "${mongodb_org_version}" | cut -d'.' -f3)" -gt "18" ]]; then
      ...
      while read -r mongodb_package; do
        ...
        if DEBIAN_FRONTEND='noninteractive' apt-get -y --allow-downgrades "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${mongodb_package}${install_mongodb_version_with_equality_sign}" &>> "${eus_dir}/logs/arm64-mongodb-downgrade.log"; then
          ...
          abort_reason="Failed to downgrade ${mongodb_package} to version ${install_mongodb_version}."
          abort
        fi
        echo -e "${WHITE_R}#${RESET} Preventing ${mongodb_package} from upgrading..."
        if echo "${mongodb_package} hold" | dpkg --set-selections; then
...
adoptium_java() {
  ...
  if curl -s "https://packages.adoptium.net/artifactory/deb/dists/" | sed -e 's/<[^>]*>//g' -e '/^$/d' -e '/\/\//d' -e '/function/d' -e '/location/d' -e '/}/d' -e 's/\///g' -e '/Name/d' -e '/Index/d' -e '/\.\./d' -e '/Artifactory/d' | awk '{print $1}' | grep -iq "${os_codename}"; then
    ...
    aptkey_depreciated
    if [[ "${apt_key_deprecated}" == 'true' ]]; then
      if wget -qO - "https://packages.adoptium.net/artifactory/api/gpg/key/public" | tee -a "${eus_dir}/logs/repository-keys.log" | gpg --dearmor | tee -a "/etc/apt/keyrings/packages-adoptium.gpg" &> /dev/null && [[ -s "/etc/apt/keyrings/packages-adoptium.gpg" ]]; then echo -e "${GREEN}#${RESET} Successfully added the key for adoptium packages! \\n"; signed_by_value_adoptium="[ signed-by=/etc/apt/keyrings/packages-adoptium.gpg ] "; else abort_reason="Failed to add the key for adoptium packages."; abort; fi
    else
      if wget -qO - "https://packages.adoptium.net/artifactory/api/gpg/key/public" | tee -a "${eus_dir}/logs/repository-keys.log" | apt-key add - &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added the key for adoptium packages! \\n"; else abort_reason="Failed to add the key for adoptium packages."; abort; fi
    fi
    ...
    { echo "# Could not find \"${os_codename}\" on https://packages.adoptium.net/artifactory/deb/dists/"; echo "# List of what was found:"; curl -s "https://packages.adoptium.net/artifactory/deb/dists/" | sed -e 's/<[^>]*>//g' -e '/^$/d' -e '/\/\//d' -e '/function/d' -e '/location/d' -e '/}/d' -e 's/\///g' -e '/Name/d' -e '/Index/d' -e '/\.\./d' -e '/Artifactory/d' | awk '{print $1}'; } &>> "${eus_dir}/logs/adoptium.log"
  fi
...
if [[ "${unifi_dependencies}" == 'fail' ]]; then
  ...
  if [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia|bionic|tara|tessa|tina|tricia|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic|noble|stretch|continuum|buster|bullseye|bookworm|trixie|forky) ]]; then
    echo -e "${WHITE_R}#${RESET} Installing binutils, ca-certificates-java and java-common..."
    if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install binutils ca-certificates-java java-common &>> "${eus_dir}/logs/apt.log"; then echo -e "${GREEN}#${RESET} Successfully installed binutils, ca-certificates-java and java-common! \\n"; else abort_reason="Failed to install binutils, ca-certificates-java and java-common in the first run."; abort; fi
    if [[ "${required_java_version}" == "openjdk-8" ]]; then
      echo -e "${WHITE_R}#${RESET} Installing jsvc and libcommons-daemon-java..."
      if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install jsvc libcommons-daemon-java &>> "${eus_dir}/logs/apt.log"; then echo -e "${GREEN}#${RESET} Successfully installed jsvc and libcommons-daemon-java! \\n"; else abort_reason="Failed to install jsvc and libcommons-daemon-java in the first run."; abort; fi
    fi
  elif [[ "${os_codename}" == 'jessie' ]]; then
    echo -e "${WHITE_R}#${RESET} Installing binutils, ca-certificates-java and java-common..."
    if DEBIAN_FRONTEND='noninteractive' apt-get -y --force-yes -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install binutils ca-certificates-java java-common &>> "${eus_dir}/logs/apt.log"; then echo -e "${GREEN}#${RESET} Successfully installed binutils, ca-certificates-java and java-common! \\n"; else abort_reason="Failed to install binutils, ca-certificates-java and java-common in the first run."; abort; fi
...
if ! systemctl start unifi; then abort_reason="Failed to start the Network Application."; abort; fi
sleep 3
